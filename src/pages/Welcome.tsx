...
const blockContent = `通过Bin智能BI平台，即使对数据分析一无所知的同学也能够轻松完成复杂的数据分析任务，大幅节约人力成本。利用AI接口和自动化技术，为您提供了快速、准确和可靠的数据洞察力。立即体验Bin智能BI平台，让数据分析变得简单而智能！`;
const BIIntroduce = `这是一个创新的数据分析工具，与传统的BI完全不同。项目利用AI技术，以完全新颖的方式帮助用户进行数据分析，节约人力成本并提高效率。

在传统的数据分析过程中，数据分析者需要耗费大量的时间和精力来导入、清理和准备数据，然后仔细选择合适的图表类型，并分析数据以得出结论。这个过程耗时且复杂，对于不擅长数据分析的用户来说更是一项挑战。

但是，这个项目将彻底改变这一现状。数据分析工具不需要用户具备专业的数据分析能力，只需导入最原始的数据集并输入所需的分析目标，即可依靠强大的AI技术自动生成符合要求的图表和详细的分析结论。这一创新的方法不仅减少了数据准备和分析的时间和工作量，还使得数据分析对于非专业用户来说变得轻而易举。

此外，项目还包含其他功能，如图表管理和异步生成。通过图表管理功能，用户可以轻松管理已生成的图表，并进行编辑和导出。而异步生成功能可以让用户在图表生成的同时进行其他操作，进一步提高工作效率。

这个项目的最大优势在于让不具备数据分析技能的人员也能快速进行数据分析。解决方案利用AI接口，根据用户输入的分析目标和原始数据，快速生成高质量的可视化图表和详细的分析结论。这将大大降低人工数据分析成本，提高工作效率，并为决策提供更有力的支持。`
const Welcome: React.FC = () => {
  return (
      <Card>
        <Alert
          message={'欢迎使用Bin智能BI平台！'}
          type="success"
          showIcon
          banner
          style={{
